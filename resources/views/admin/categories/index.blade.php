...
<div class="space-y-5">
    ...
    <form method="GET" action="{{ route('admin.categories.index') }}" class="flex gap-3">
        <div class="flex-1 relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kategori..."
                   class="w-full pl-10 pr-4 py-2.5 rounded-xl bg-navy-surface border border-navy-border text-sm text-ink-primary placeholder:text-ink-muted focus:border-coral/40 focus:ring-1 focus:ring-coral/30 focus:outline-none">
        </div>
                        ...
                        <tr class="hover:bg-white/[0.02]">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-medium text-ink-primary">{{ $cat->name }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-4 text-ink-secondary">{{ $cat->events_count ?? 0 }} event</td>
                            <td class="px-5 py-4 text-ink-secondary">{{ $cat->created_at->isoFormat('D MMM YYYY') }}</td>
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('admin.categories.edit', $cat) }}" class="text-ink-secondary hover:text-coral transition-colors text-xs">Edit</a>
                                    <form method="POST" action="{{ route('admin.categories.destroy', $cat) }}" class="inline"
                                          onsubmit="return confirm('Hapus kategori \'{{ addslashes($cat->name) }}\'? Event terkait tidak akan dihapus.')">
                                        @csrf @method('DELETE')
    ...
    @else
        <div class="flex flex-col items-center justify-center py-16 rounded-xl bg-navy-surface border border-navy-border border-dashed">
            ...
            <a href="{{ route('admin.categories.create') }}" class="text-sm text-coral hover:underline">Tambah kategori pertama</a>
        </div>
    @endif
</div>
